...
int query_in_pgtbl(void *pgtbl, vaddr_t va, paddr_t *pa, pte_t **entry)
{
        ...
        for (int i = 0; i <= lowest_level; i++) {
                ptp_t *next_ptp;
                BUG_ON(cur_ptp == NULL);
                int res = get_next_ptp(cur_ptp, i, va, &next_ptp, &pte, false);
                if (res < 0) return res;
                cur_ptp = next_ptp;
        }

        if (pa != NULL)
                *pa = (pte->l3_page.pfn << PAGE_SHIFT) + (va & PAGE_MASK);
        if (entry != NULL)
                *entry = pte;
        /* BLANK END */
        /* LAB 2 TODO 4 END */
                       ...
                       vmr_prop_t flags, int kind)
{
        ...
        for (int i = 0; i < len; i += PAGE_SIZE) {
                ...
                ptp_t *cur_ptp = pgtbl;
                pte_t *pte = NULL;
                for (int j = 0; j <= lowest_level; j++) {
                        BUG_ON(pgtbl == NULL);
                        ptp_t *next_ptp;
                        int res = get_next_ptp(cur_ptp, j, va, &next_ptp, &pte, true);
                        if (res < 0) return res;
                        cur_ptp = next_ptp;
                }
...
int unmap_range_in_pgtbl(void *pgtbl, vaddr_t va, size_t len)
{
        ...
        for (int i = 0; i < len; i += PAGE_SIZE) {
                ...
                for (int j = 0; j <= lowest_level; j++) {
                        ptp_t *next_ptp;
                        int res = get_next_ptp(cur_ptp, j, va, &next_ptp, &pte, false);
                        cur_ptp = next_ptp;
                        if (res < 0) return res;
                }
