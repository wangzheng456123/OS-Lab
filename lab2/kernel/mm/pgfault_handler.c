...
int handle_trans_fault(struct vmspace *vmspace, vaddr_t fault_addr)
{
        ...
        case PMO_SHM: {
                ...
                if (pa == 0) {
                        ...
                        // kinfo("the pa for it is zero, current index is %d\n", index);
                        void* page = get_pages(0);
                        BUG_ON(page == NULL);
                        memset(page, 0, PAGE_SIZE);
                        pa = virt_to_phys(page);

                        // kinfo("allocated pa is %lx\n", pa);
                        /* BLANK END */
                        /*
                        ...
                        lock(&vmspace->pgtbl_lock);
                        /* BLANK BEGIN */
                        // kinfo("current fault addr is : %lx\n", fault_addr);
                        int ret = map_range_in_pgtbl(vmspace->pgtbl, fault_addr, pa, PAGE_SIZE, perm);
                        pte_t *pte;
                        query_in_pgtbl(vmspace->pgtbl, fault_addr + 0x50, &pa, &pte);
                        // kinfo("get pte %lx\n", pa);
                        BUG_ON(ret != 0);
                        // kinfo("pa has been mapped!\n");
                        /* BLANK END */
                        unlock(&vmspace->pgtbl_lock);
                } else {
                        /*
                        ...
                        if (pmo->type == PMO_SHM || pmo->type == PMO_ANONYM) {
                                ...
                                lock(&vmspace->pgtbl_lock);
                                /* BLANK BEGIN */
                                // kinfo("the pa is not zero, map it now!\n");
                                int ret = map_range_in_pgtbl(vmspace->pgtbl, fault_addr, pa, PAGE_SIZE, perm);
                                BUG_ON(ret != 0);
                                /* BLANK END */
                                /* LAB 2 TODO 7 END */
