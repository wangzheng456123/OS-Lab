...
static void choose_new_current_slab(struct slab_pointer *pool)
{
        ...
        list_del(&current_slab->node);
        if (!list_empty(&pool->partial_slab_list)) {
                current_slab = list_entry(&pool->partial_slab_list, struct slab_header, node);
        }
        else {
...
static void *alloc_in_slab_impl(int order)
{
         ...
         */
        while (current_slab != NULL && current_slab->current_free_cnt == 0) {
        ...
        BUG_ON(current_slab->current_free_cnt && free_list == NULL);
        if (current_slab->current_free_cnt && current_slab->current_free_cnt == current_slab->total_free_cnt - 1) {
                list_add(&current_slab->node, &slab_pool[order].partial_slab_list);
        }
        /* BLANK BEGIN */
