...
struct vmregion *find_vmr_for_va(struct vmspace *vmspace, vaddr_t addr)
{
        ...
        /* BLANK BEGIN */
        comp_key_func cmp_func = cmp_vmr_and_va;

        struct rb_root *vmr_root = &vmspace->vmr_tree;
        struct rb_node *vmr_tree_node = rb_search(vmr_root, (void*)addr, cmp_func);

        if (vmr_tree_node == NULL) return NULL; 

        return rb_entry(vmr_tree_node, struct vmregion, tree_node); 
        /* BLANK END */
        /* LAB 2 TODO 6 END */
}
