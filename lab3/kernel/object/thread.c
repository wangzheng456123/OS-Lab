...
void create_root_thread(void)
{
        ...
        for (int i = 0; i < meta.phnum; i++) {
                ...
                memcpy((void *)phys_to_virt(segment_pmo->start), (void *)((unsigned long)&binary_procmgr_bin_start
                        + offset + ROOT_BIN_HDR_SIZE), filesz);
                /* LAB 3 TODO END */
        
                unsigned vmr_flags = 0;    
                /* LAB 3 TODO BEGIN */
                /* Set flags*/
                vmr_flags |= (flags & PHDR_FLAGS_R) >> 2;
                vmr_flags |= (flags & PHDR_FLAGS_X) << 2;
                vmr_flags |= (flags & PHDR_FLAGS_W);
                /* LAB 3 TODO END */
