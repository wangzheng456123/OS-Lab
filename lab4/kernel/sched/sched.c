...
struct thread *find_runnable_thread(struct list_head *thread_list)
{
        ...
        for_each_in_list(cur_thread, struct thread, ready_queue_node, thread_list) {
                if (!cur_thread->thread_ctx->is_suspended && 
                (cur_thread->thread_ctx->kernel_stack_state == KS_FREE ||
